{{-- resources/views/admin/manage_datalake_index.blade.php --}}
...
<div class="card card-flush shadow-sm p-5">
  <div class="d-flex justify-content-between align-items-center mb-6">
    ...
    <a href="{{ url('/admin/manage_datalake/create') }}" class="btn btn-danger">Add Ingestion</a>
  </div>
        ...
        <tbody>
          @forelse($rows as $r)
            <tr>
              <td>{{ $r->id }}</td>
              <td>
                <form action="{{ url('/admin/manage_datalake/'.$r->id.'/toggle') }}" method="POST">
                  @csrf @method('PATCH')
              ...
              <td>{{ $r->next_run_at ?? '-' }}</td>
              <td class="text-end">
                <form action="{{ route('queue_datalake', $r->id) }}" method="POST" class="d-inline">
                  @csrf @method('PATCH')
                ...
                <a class="btn btn-sm btn-light" href="{{ url('/admin/manage_datalake/'.$r->id.'/edit') }}">Edit</a>
                <form action="{{ url('/admin/manage_datalake/'.$r->id) }}" method="POST" class="d-inline"
